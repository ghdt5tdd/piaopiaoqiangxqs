...
const IMG_PREFIX = 'https://sping-cloud-fall.oss-cn-shanghai.aliyuncs.com/wlhn/'
function handleImgUrl (obj, imgKey) {
  if(obj instanceof Array) {
    obj.forEach(v => {
      if (v[imgKey] && v[imgKey].indexOf(IMG_PREFIX) === -1) {
        v[imgKey] = IMG_PREFIX + v[imgKey]
      }
    })
  }else if (obj instanceof Object) {
    if (obj[imgKey] && obj[imgKey].indexOf(IMG_PREFIX) === -1) {
      obj[imgKey] = IMG_PREFIX + obj[imgKey]
    }
  }else {
